...
- Added the `deep-research` Skill for bounded multi-source research, per-claim citations, contradiction tracking, and optional public X evidence through Xquik MCP.
- Added a ready remote Xquik MCP entry that uses client-managed OAuth discovery.
...
- Added repeatable `--server` selection to MCP planning and application so one ready entry can be used without selecting unrelated placeholders.
...
- Placeholder protection now evaluates only the explicitly selected MCP entries and still blocks any selected entry with unresolved credentials.
